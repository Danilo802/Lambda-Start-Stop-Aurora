        ...
        if statuscluster == "stopped":         
            ...
            for tag in tags:
                if tag["Key"] == "TAG_START_NAME":
                    if tag["Value"] == "True":
                        valortag = tag["Value"]
        if clustername == "db-master-teste":
            valortag == "True"

        if valortag == "True" and clustername != "db-master-teste":
            rds_client = boto3.client("rds",region_name="sa-east-1") 
            resposta = rds_client.start_db_cluster(DBClusterIdentifier = clustername)
